...
\AfterCalculatingTypearea{%
  ...
  \KOMAoptions{headsepline=:text}% <- added
}
\recalctypearea
...
\def\Venv#1{\index{Environment Verbatim!#1@\texttt{#1}}}
\def\Wenv#1{\index{Environment Verbatim Write!#1@\texttt{#1}}}
...
% Important words 9, files used in examples
    ...
    keywords=[10]{test-fig-1981.tex,test-fig-1981.pdf, test-fig-1,test-fig-2,test-out,test-in.ltx,%
...
\lstset{
    ...
    keywords=[20]{--margins, -recorder,-shell-escape,-s,-o,-e,-p,-j,--imgdir,--luatex,--latexmk,--svg,--png,--nopdf,pics},%
...
\begin{abstract}
    \texttt{\myscript} run a \TeX{} source, read all \Lenv{postscript}, \Lenv{pspicture}, \Lenv{psgraph}
    and \Lenv{PSTexample} environments and convert in images \texttt{pdf,eps,jpg,svg}
...
If the \meta[cf=optcolor]{input file} contains files using \Lcs{input}\marg[type=tt]{file} or
...
\meta[cf=optcolor]{output file}, \Lscript{pst2pdf} need a various steps. Let's assume that the %
\meta[cf=optcolor]{input file} is \LFile{test.tex}, \meta[cf=optcolor]{output file} is %
...
Note the options passed to \meta[type=tt,cf=ForestGreen,ac=lightgray]{compiler} always use \prgopt*{recorder}\prgopt*{shell-escape}.
...
and generate \Lext{pdf} files, then runing:
...
\item Execute the script using \cmdopt{norun}, move to \texttt{./images} and generate \Lext{dvi} or \Lext{xdv} file, then runing:
...
[user@machine~:]§ `\small\Lscript{pdfcrop} \oarg[type=tt,cf=gray,sbc=optcolor,ac=gray]{options}` test-fig-all.dvi
\end{examplecmd}
