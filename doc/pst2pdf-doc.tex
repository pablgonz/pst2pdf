...
\begin{examplecmd}
[user@machine ~:]§ `\small\meta[type=tt,cf=ForestGreen,ac=lightgray]{compiler}` -recorder -no-shell-escape test-pdf.tex
\end{examplecmd}
...
\DescribeCmd[np]{noprew}{boolean}{off}
...
\DescribeCmd[ni]{norun}{boolean}{off}
...
\DescribeCmd[ns]{nosource}{boolean}{off}
Don't create standalone files.

\DescribeCmd[x]{xetex}{boolean}{off}
Using \Lprog{xelatex} compiler \meta[cf=optcolor]{input file} and \meta[cf=optcolor]{output file}.
In the execution \Lprog{xelatex} is called with the \prgopt*{no-pdf} option
generating a \Lext{xvd} file and then it is processed using \Lprog{xdvipdfmx},
...
\item Execute the script using \cmdopt{norun}, move to \texttt{./images} and generate \Lext{pdf} files runing:
